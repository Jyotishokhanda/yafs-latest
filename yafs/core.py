...
class Sim:
    ...
    **Main variables to coordinate with algorithm:**
    ...
    # self.__send_message(app_name, message, idDES, self.SOURCE_METRIC)
        ...
        # self.test_sensor = 0
        # for node,val in self.topology.nodeAttributes.items():
        ...
        try:
            ...
            if DES_dst == [None] or DES_dst == [[]]:
                ...
                if self.logger.isEnabledFor("Debug"):
                    ...
                    self.logger.debug("NODES (%i)" %
                                      len(self.topology.G.nodes()))
                ...
                self.logger.debug("(#DES:%i)\t--- SENDING Message:\t%s: PATH:%s  DES:%s" %
    ...
    def __network_process(self):
        ...
        while not self.stop:
            ...
            if not message.path or message.path[-1] == message.dst_int or len(message.path) == 1:
                ...
                pipe_id = "%s%s%i" % (
                    message.app_name, message.dst, message.idDES)
            ...
            try:
                ...
                transmit = size_bytes / \
                    (self.topology.get_edge(link)[
                        Topology.LINK_BW] * 1000000.0)  # MBYTES!
    ...
    def __add_placement_process(self, placement):
        ...
        self.logger.debug(
            "Added_Process - Placement Algorithm\t#DES:%i" % myId)
        while not self.stop and self.des_process_running[myId]:
            yield self.env.timeout(placement.get_next_activation())
            placement.run(self)
            self.logger.debug("(DES:%i) %7.4f Run - Placement Policy: %s " %
                              (myId, self.env.now, self.stop))  # Rewrite
    ...
    def __add_population_process(self, population):
        ...
        self.logger.debug(
            "Added_Process - Population Algorithm\t#DES:%i" % myId)
        while not self.stop and self.des_process_running[myId]:
            yield self.env.timeout(population.get_next_activation())
            self.logger.debug("(DES:%i) %7.4f Run - Population Policy: %s " %
                              (myId, self.env.now, self.stop))  # REWRITE
            population.run(self)
        self.logger.debug(
            "STOP_Process - Population Algorithm\t#DES:%i" % myId)
    ...
    def __add_source_population(self, idDES, name_app, message, distribution):
        ...
        """
        self.logger.debug(
            "Added_Process - Module Pure Source\t#DES:%i" % idDES)
    ...
    def __update_node_metrics(self, app, module, message, des, type):
     ...
     #
            ...
            if  message.path[0] == self.test_sensor:
                ...
                # print("service_time = ",time_service)
                # print("waiting_time = ",waiting_time)
                # print("actual_latency = ",final_latency)
                if self.baseline:
    ...
    def __add_down_node_process(self, next_event, **param):
        ...
        self.des_process_running[myId] = True
        self.logger.debug(
            "Added_Process - Down entity Creation\t#DES:%i" % myId)
        while not self.stop and self.des_process_running[myId]:
            yield self.env.timeout(next_event(**param))
            self.logger.debug("(DES:%i) %7.4f Node " % (myId, self.env.now))
                        ...
                        if not register["message_out"]:
                            ...
                            if register["dist"](**register["param"]):
                                if not register["module_dest"]:
                                    ...
                                    msg_out = copy.copy(
                                        register["message_out"])
                                    msg_out.timestamp = self.env.now
                                    msg_out.id = msg.id
                                    msg_out.last_idDes = copy.copy(
                                        msg.last_idDes)
                                    ...
                                    msg_out.last_idDes = copy.copy(
                                        msg.last_idDes)
                                    msg_out.id = msg.id
                                    msg_out.last_idDes = msg.last_idDes.append(
                                        ides)
    ...
    def __add_stop_monitor(self, name, function, distribution, show_progress_monitor, **param):
        ...
        self.logger.debug(
            "STOP_Process - Internal Monitor: %s\t#DES:%i" % (name, myId))
    ...
    def __add_consumer_service_pipe(self, app_name, module, idDES):
        ...
        self.consumer_pipes["%s%s%i" %
                            (app_name, module, idDES)] = simpy.Store(self.env)
        ...
        if self.until:
            ...
            if self.env.now >= self.until:
                ...
                if show_progress_monitor:
                    self.pbar.close()
                self.logger.info(
                    "! Stop simulation at time: %f !" % self.env.now)
    ...
    def deploy_monitor(self, name, function, distribution, **param):
        ...
        """
        self.env.process(self.__add_monitor(
            name, function, distribution, **param))
    ...
    def register_event_entity(self, next_event_dist, event_type=EVENT_UP_ENTITY, **args):
        ...
        if event_type == EVENT_UP_ENTITY:
            self.env.process(self.__add_up_node_process(
                next_event_dist, **args))
        elif event_type == EVENT_DOWN_ENTITY:
            self.env.process(self.__add_down_node_process(
                next_event_dist, **args))
    ...
    def deploy_source(self, app_name, id_node, msg, distribution):
        ...
        self.env.process(self.__add_source_population(
            idDES, app_name, msg, distribution))
        self.alloc_DES[idDES] = id_node
        self.alloc_source[idDES] = {
            "id": id_node, "app": app_name, "module": msg.src, "name": msg.name}
        return idDES
    ...
    def __deploy_source_module(self, app_name, module, id_node, msg, distribution):
        ...
        idDES = self.__get_id_process()
        self.des_process_running[idDES] = True
        self.env.process(self.__add_source_module(
            idDES, app_name, module, msg, distribution))
        self.alloc_DES[idDES] = id_node
        return idDES
    ...
    def __deploy_module(self, app_name, module, id_node, register_consumer_msg):
        ...
        idDES = self.__get_id_process()
        self.des_process_running[idDES] = True
        self.env.process(self.__add_consumer_module(
            idDES, app_name, module, register_consumer_msg))
    ...
    def deploy_app(self, app, placement, population, selector):
        ...
        if not placement.name in self.placement_policy.keys():  # First Time
            self.placement_policy[placement.name] = {
                "placement_policy": placement, "apps": []}
        ...
        if not population.name in self.population_policy.keys():  # First Time
            self.population_policy[population.name] = {
                "population_policy": population, "apps": []}
    ...
    def deploy_module(self, app_name, module, services, ids):
        ...
        if len(register_consumer_msg) > 0:
            for id_topology in ids:
                id_DES.append(self.__deploy_module(
    ...
    def get_assigned_structured_modules_from_DES(self):
        ...
        for app in self.alloc_module:
            for module in self.alloc_module[app]:
                ...
                for des in deployed:
                    fullAssignation[des] = {
                        "DES": self.alloc_DES[des], "module": module}
    ...
    def __add_mobile_agent(self, ides, gme):
        ...
        self.logger.info(
            "(#DES:%i)\t--- Mobile Entity STARTS :\t%s " % (ides, gme._id))
        while (len(gme.path) - 1 > gme.current_position) and not self.stop and self.des_process_running[ides]:
            e = (gme.path[gme.current_position],
                 gme.path[gme.current_position + 1])
            data = self.street_network.get_edge_data(*e)
            try:
                next_time = int(utils.toMeters(
                    data[0]["geometry"]) / gme.speed)
            except KeyError:
            ...
            self.logger.info("(#DES:%i)\t--- DO ACTION :\t%s " %
                             (ides, gme._id))
            gme.do.action(gme)
        ...
        self.logger.info(
            "(#DES:%i)\t--- Mobile Entity ENDS :\t%s " % (ides, gme._id))
    ...
    def generate_animation(self, pathFile):
        ...
        animation.make_video(output_file=pathFile,
                             framerate=10, linewidth=1.0, G=self.topology.G)
    ...
    def my_proc(self):
        while True:
            ...
            if self.env.now % 40 == 0:
                ...
                #     if one_link not in all_links :
                #         continue
                #     band_val = self.topology.get_edge(one_link)[self.topology.LINK_BW]
                #     pr_val = self.topology.get_edge(one_link)['PR']
                #     current_bandwidths[edge_node] = (band_val,band_val)
                #     current_prs[edge_node] = pr_val
                #     memories[edge_node] = (self.topology.nodeAttributes[edge_node]["peak_memory"],self.topology.nodeAttributes[edge_node]["residual_memory"])
                ...
                self.apps["SimpleCase"].messages["M.A"].inst = random.choice(
                    ipt_array)
                for key, val in self.apps["SimpleCase"].messages.items():
                    if val.src == "Sensor":
                        val.bytes = random.choice(input_size_array)
                        # temp_inst = random.choice(ipt_array) 
                        # val.inst = ((temp_inst * val.bytes)/26000)
                        val.inst = random.choice(ipt_array)
                        # print(val.inst)

                for edge_node in id_cluster:
                    self.topology.nodeAttributes[edge_node]["device_bandwidth"] = random.choice(
                        band_array)
                    random_peak = random.choice(peak_memories)
                    self.topology.nodeAttributes[edge_node]["peak_memory"] = random_peak
                    self.topology.nodeAttributes[edge_node][
                        "residual_memory"] = self.topology.nodeAttributes[edge_node]["RAM"] - random_peak
                # print("node:")
                # print(edge_node)
                # print(self.topology.nodeAttributes[edge_node]["device_bandwidth"])
                #print self.topology.nodeAttributes[edge_node]["IPT"]
    ...
    # def change_topology(self, selector_path):
    ...
    #             temp_topology = {}
    #             temp_topology["entity"] = self.iterate_dict(
    #                 self.topology.get_nodes_att())
    #             temp_topology["link"] = []
    #             temp_topology_1, alloc_des = selector_path.create_dynamic_links(
    #                 temp_topology, self.alloc_DES)
    #             self.alloc_DES = alloc_des

    #             self.topology.load(temp_topology_1)
    ...
    def run(self, until, selector_path, test_initial_deploy=False, show_progress_monitor=True, mobile_behaviour=False):
        ...
        # self.env.process(self.change_topology(selector_path))
        self.env.process(self.my_proc())
        ...
        *Simpy.run.until* wait to all pipers are empty. So, hundreds of messages should be service... We force with the stop
        ...
        self.env.process(self.__add_stop_monitor("Stop_Control_Monitor", self.__ctrl_progress_monitor,
                                                 distribution, show_progress_monitor, time_shift=time_shift))
